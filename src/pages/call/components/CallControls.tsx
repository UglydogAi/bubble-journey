...
const CallControls: React.FC<CallControlsProps> = ({ 
  ...
  return (
    <motion.div 
      ...
      className="flex gap-6 sm:gap-8 mt-4"
    >
      <Button
        variant="outline"
        size="lg"
      ...
      <Button
        ...
        size="lg"
        className="w-14 h-14 sm:w-16 sm:h-16 rounded-full relative overflow-hidden
                 border-2 border-yellow-400/50 bg-transparent hover:bg-yellow-500/10
                 transition-transform duration-200 hover:scale-105"
      >
        {true ? (
          <Volume2 className="w-6 h-6 sm:w-7 sm:h-7 text-yellow-400" />
        ) : (
